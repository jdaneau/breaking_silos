function tutorial_popup(_x,_y,tutorial_id){
	...
	switch(tutorial_id) {
		...
		case TUTORIAL.PROJECT_TIME:
			tut.text = "Nice, you just placed a measure! Note that every measure has a cost and must fit within the budget. Measures also have an implementation time, which varies from weeks to years. Because of this, most measures may not be fully implemented by the next round. A lot of them are pretty long-term and the next disaster may strike sooner!"
		break;
		case TUTORIAL.OPEN_CALCULATOR:
			tut.text = "As the finance manager, you have the unique job of keeping track of expenses. Click the \"Open Calculator\" button to take a look at the calculator tool."
		break;
		...
		case TUTORIAL.LAYER_POPULATION:
			tut.text = "The population layer shows how many people live in each cell, by the thousand. Naturally, most people choose to live around the capital region. You may want to prioritize these high-population areas as they are the most vulnerable to population loss!"
		break;
		...
		case TUTORIAL.LAYER_WATERSHEDS:
			tut.text = "Watershed ares show the reach of each river system, along with its direction of flow. When placing dams, the downstream tiles will have their flood chance lowered and flood damage in general will be mitigated. You can only build one dam per river system!"
		break;
		case TUTORIAL.LAYER_HISTORY:
			tut.text = "Flood and drought history represents the indigenous knowledge of the citizens living in the country. These are not official maps, but signify areas that people (and thus voters) believe are vulnerable and should be protected. You may want to convince the president to protect these areas first."
		break;
		case TUTORIAL.LAYER_AGRICULTURE:
			tut.text = "Agricultural areas are cells that have crops planted in them. To keep the country fed, the number of agricultural cells should never be allowed to decrease. As such, if you see any destroyed agricultre, you should point out that they need to be replanted!"
		break;
		case TUTORIAL.MEASURE_RELOCATE:
			tut.text = "When placing the relocate population measure, you need to put it on the source cell from which you want the population to come out. The destination cells are randomly chosen based on general appeal of the movers. They will tend not to move into cells that are damaged or have low hospital/airport access."
		break;
		case TUTORIAL.MEASURE_DAM:
			tut.text = "Dams are expensive and long-term projects, so make sure you are placing it in a good place! Only one dam can exist per watershed. Cells that are downstream from a dam will have much lower flood risk, though may have a slightly higher drought risk due to a decrease of water."
		break;
		case TUTORIAL.MEASURE_EVACUATE:
			tut.text = "To evacuate population, place this measure on a cell where you wish to construct temporary shelters. Evacuated population will be removed from all the affected cells to this cell. For effective evacuation, you should evacuate to as many cells as were affected by the disaster."
		break;
	}
