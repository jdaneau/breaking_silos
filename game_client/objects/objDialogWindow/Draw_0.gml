draw_set_alpha(0.75)
draw_color_rectangle(0,0,room_width,room_height,c_black,false)
draw_set_alpha(1)

draw_self()

draw_set_font(fChat)
draw_set_halign(fa_center)
draw_set_valign(fa_top)

draw_text_ext(x+(sprite_width/2),16,prompt,font_get_size(fChat),sprite_width-32)

if question {
	while string_width(keyboard_string) > (sprite_width-32) {
		keyboard_string = string_copy(keyboard_string,1,string_length(keyboard_string)-1)	
	}
	draw_gui_border(x+16,y+sprite_height-64,x+sprite_width-16,y+sprite_height-48)
	var textX = x+16+4;
	var textY = y+sprite_height-64+4
	draw_set_halign(fa_left)
	draw_text(textX,textY,keyboard_string)
	var str_end = textX + string_width(keyboard_string) + 4;
	if line_flash and str_end < (x+sprite_width-16) {
		draw_line_color(str_end,textY,str_end,textY+string_height(keyboard_string),c_black,c_black)
	}
	
	var buttonX = x+(sprite_width/2)-64;
	var buttonY = y+sprite_height-40;
	var hover = coords_in(mouse_x,mouse_y,buttonX,buttonY,buttonX+128,buttonY+32);
	if hover {
		draw_color_rectangle(buttonX,buttonY,buttonX+128,buttonY+32,c_ltgray,false)
		if mouse_check_pressed(mb_left) {
			confirm(keyboard_string)
			instance_destroy()
		}
	}
	draw_gui_border(buttonX,buttonY,buttonX+128,buttonY+32)
	draw_set_halign(fa_center)
	draw_set_valign(fa_middle)
	draw_text(x+(sprite_width/2),buttonY+16,"Confirm")
} else {
	var width = sprite_width-32;
	var sep_width = 8;
	var button_width = (width - sep_width*(array_length(options)-1)) / array_length(options);
	for(var i=0; i<array_length(options); i++) {
		var drawX = x+16 + (i * button_width);
		if i > 1 { drawX += (i-1) * sep_width }
		var drawY = y+sprite_height-64;
		var hover = coords_in(mouse_x,mouse_y,drawX,drawY,drawX+button_width,drawY+48);
		if hover {
			draw_color_rectangle(drawX,drawY,drawX+button_width,drawY+48,c_ltgray,false)
			if mouse_check_pressed(mb_left) {
				confirm(options[i])
				instance_destroy()
			}
		}
		draw_gui_border(drawX,drawY,drawX+button_width,drawY+48)
		draw_set_halign(fa_center)
		draw_set_valign(fa_middle)
		draw_text(drawX+(button_width/2),drawY+24,options[i])
	}
}