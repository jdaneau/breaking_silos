...
switch global.state.role {
	case ROLE.PRESIDENT:
		...
		array_push(text, " - focus on ensuring a strong national economy, realising that it depends on both the small wealthy segment of society as well as the large agricultural sector")
	...
	case ROLE.AGRICULTURE:
		...
		array_push(text, " - focus on short term to minimise impacts on the agriculture sector")
		...
		array_push(text, " - like the idea of those drought-resistant crops to secure enough food but also realise that distributing such crops can be expensive")
	...
	case ROLE.CITIZEN:
		...
		array_push(text, " - are against the construction of dams as they can create droughts downstream")
	...
	case ROLE.HOUSING:
		...
		array_push(text, " - focus on medium term measures, as there is a big need for urban expansion due to population growth")
